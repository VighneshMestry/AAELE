...
const genAI = new GoogleGenerativeAI("AIzaSyBrIph-OSVnPnbeq11CvRpQEv4irTY6yCU");
...
const processVideo = async (videoUrl) => {
    ...
    try {
        ...
        let prompt = `
        Consider that you are a subject matter expert and this is a video of a live lecture. You have to create structured and detailed study notes based on the content of this lecture. The notes should be comprehensive, clear, and easy to understand for students. Follow these guidelines:

        1. **Introduction**: Provide a brief overview of the lecture's topic, including the key objectives or learning outcomes.
  
        2. **Main Concepts**: Organize the main concepts discussed in the lecture in a logical order. Break them down into sub-sections where necessary. Ensure that each concept is clearly defined and explained with relevant examples.

        3. **Formulas and Equations**: If any formulas, equations, or mathematical principles are introduced during the lecture, make sure to list them separately. Provide a clear explanation of each formula and how it’s used with any example applications, if applicable.

        4. **Important Points**: Highlight critical points and key takeaways. These should include definitions, processes, steps, or principles that students should remember.

        5. **Real-World Applications**: Mention any real-world examples or applications of the concepts discussed in the lecture, if they are provided during the session. Relating theory to practice helps reinforce learning.

        6. **Additional Resources**: If any additional resources, references, or reading materials are suggested during the lecture, mention them here for students who want to explore further.

        Ensure the notes are well-organized, easy to follow, and concise. The goal is to create a study guide that can be referenced for reviewing the material effectively.

        Please make the notes readable and structured, using bullet points, headings, subheadings, and numbering as appropriate.

            Please do not include any other phrases like "Sure here are the notes", "the teacher said" in the output. The output should consist of only and only notes`;
...
router.post('/get_note', async (req, res) => {
    try {
        ...
        if (!notes) {
            return res.status(200).send({ message: 'Notes retrieved successfully', notes: [] });
        }
